...
def count_movies_from_given_year(movies, year):
    counter = 0
    for movie in movies:
        if movie.year == year:
            counter += 1
...
def select_movies_with_given_actor(movies, actor):
    result_movies = []
    for movie in movies:
...
def sum_movie_duration_in_given_period(movies, start_year, end_year):
    total_duration = 0
    for movie in movies:
...
def get_directors_of_given_actor(movies, actor):
    directors = []
    for movie in movies:
...
def find_string_starting_with_given_letter(strings, letter):
    for string in strings:
        if len(string) != 0 and string[0].lower() == letter:
            return string
    return None

def find_number_greater_than(numbers, given_number):
